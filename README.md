...
- **Пользовательские шаблоны поиска ссылок** (по умолчанию поддерживает форматы: чистые URL(`https://...`), Markdown(`[текст](url)`)
...
Шаблоны, используемые для поиска ссылок в тексте.
Располагаются в порядке применения в процессе поиска (сначала сканируется 1-ый шаблон, потом 2-ой, и т.д.).
Нулевая группа используется для обозначения границ замены.
Первая группа используется для вычисления URL-адреса.

**По умолчанию заданы:**
- `\[[^\]]+\]\(([^\)]+)\)` - Markdown-шаблон
- ```(https?:\/\/[^\s<>"{}|\\^`\[\]]+[[\w]|\/](?=[\s!.,;:?()\[\]{}"'-]+|$))``` - чистый URL-адрес
